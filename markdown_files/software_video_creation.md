...
- [Video creation](#video-creation)
	...
	- [Compositing](#compositing)
		...
		- [Voorgrond](#voorgrond)
	- [Beeldmaterial maken met Unity](#beeldmaterial-maken-met-unity)
	...
	- [Converteren naar hap](#converteren-naar-hap)
...
Voor elk thema hebben we 6 video files nodig:
...
- De video moet loopen (eerste en laatste frame zijn hetzelfde)
- Bij voorkeur is elke video 2048x1024 pixels
- Bij voorkeur is de framerate 30 afbeeldingen per seconde(fps)

### Bestandsformaat

Alle 6 van deze files moeten van het .mov formaat zijn en daarbovenop moeten ze in het [hap](https://hap.video/) formaat geëncodeerd worden. Het voordeel van het `hap` formaat is dat het efficient kan afgespeeld worden, hierdoor heeft de projectie hardware lagere performantie vereisten. Het formaat kan ook goed om met transparantie in de video files wat we nodig hebben voor de `interactie` en `voorgrond` video files.
...
De drie videos gaan op elkaar getoond worden door de [projectie software](./software_video_map_NL.md), dit process van samenvoegen heet compositing.
...
### Interactie
De interactie laag is de tussenlaag waar het kind controle over zal hebben, deze laag zit bovenop de achtergrond, zorg er dus voor dat je transparantie gebruikt.

![Interactie](./images/interaction.png)

### Voorgrond
De voorgrond laag is bovenop de interactie laag en geeft meer diepte aan de projectie door nog animatie bovenop de interactie laag mogelijk te maken.

![Voorgrond](./images/foreground.png)


## Beeldmaterial maken met Unity

Deze tutorial gaat enkel over de basis van Unity heb je interesse om er meer over te leren kan je de officiële cursussen volgen: https://learn.unity.com/
...
Je kan een startproject vinden in de folder `unity_base_project`. Je kan ook je eigen project aanmaken maar dit startproject staat al vooraf ingesteld met de nodige dependencies en configuraties.
...
Je kan ook kijken naar de scene in `Scenes/Validation/VideoTestScene` deze scene is opgezet om je finale .mov files uit te testen en valideren binnenin Unity.

### Een simpele animatie maken

Omdat Unity een game engine is kan je animatie maken via code maar ook via een timeline zoals andere animatiessoftware. In deze handleiding beperken we ons tot het gebruiken van een timeline, voor meer info over hoe je met code animeert zie: https://learn.unity.com/
...
2. Je kan nu het object bewegen met de gizmo ![Gizmo](./images/Gizmo.png) in de `Scene` view of via de `Transform` in de `Inspector` window. ![Animatie bouwen](./images/Animatie.png)
...
2. Alles staat al vooraf correct ingesteld je kan op `Start	recording` drukken.
![Recorder window](./images/Recorder.png)
3. Afhankelijk van hoe zwaar je animatie is kan het even duren, als je animatie uitgespeeld is kan je drukken op `Stop recording`.
...
Een rijk gevulde scene met veel mooie modellen maken de projectie een mooi spektakel. 

Je kan deze modellen zelf maken met software als Blender maar je kan ook online mooie modellen vinden op:
...
Voorbeeld Sketchfab CreativeCommons: ![Sketchfab CreativeCommons](./images/License.png)
...
ffmpeg kan je installeren op Mac, Windows of Linux, de installers kan je hier vinden: https://ffmpeg.org/download.html


### Een bestaande video omzetten

Voor achtergronden kan je bijna alles gebruiken van video material omdat transparantie niet nodig is hier.
...
> ℹ️ Tip: Sommige online footage zijn green screens, video met een effen achtergrond deze kan je via chroma keying in video software alsnog omzetten naar een transparantie layer geschikt voor de interactie of voorgrond lagen.

### Een afbeeldingen sequentie omzetten

Heb je zelf een afbeeldingen sequentie uitgerendered met Unity of andere software?
Je kan dan deze afbeeldingen in een enkele stap samenvoegen en converteren naar een video van het juiste formaat.
